...
// 2. dispatcher.xml에 xml 등록 방식으로 bean 등록
// springweb.a01_basic.A01_StartCtrl
// springweb.a01_basic.A01_StartCtrl

@Controller
public class A01_StartCtrl {
	// http://localhost:port번호/springweb/start.do
	// 호출될 메서드.. annotation으로 RequestMapping("마지막 호출명)
	@RequestMapping("/start.do")
	public String start() {
		// 1. 요청값..
		// 2. business logic 처리 (service를 통해서 model 처리)
		// 3. view단 호출
	...
	public String sendForw(Member mem, Model d) {
		...
		d.addAttribute("msg", "id " + mem.getId() + " 이고, password " + mem.getPass() + " 입니다!!");
		// view단 : ${msg}
